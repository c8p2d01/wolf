...
void	offset_setting(double xdelta, double ydelta, t_var *data)
{
	...
	if (data->config.ray_style == 0)
	{
		...
		printf("Setting only adjustable, whn RAY_STYLE is set to rainbow mode\n");
}
...
void	width_setting(double xdelta, double ydelta, t_var *data)
{
	...
	{
		data->rays = ft_realloc(data->rays, data->config.width * sizeof(t_ray), \
					(int)ydelta * sizeof(t_ray));
	}
